...
impl MemoryViewer {
        ...
        for (idx, stored_byte) in self.bytes.iter_mut().enumerate() {
            ...
            if *stored_byte != new_byte {
                *stored_byte = new_byte;
                ...
                self.changed_highlights.insert(
                    self.base_address + idx as u64, now + HIGHLIGHT_DURATION
                );
            }
        }
    ...
    pub fn jump_absolute_at_highlighted(&mut self) {
        let addr = match is_32() {
            true => read_from_slice::<u32>(&self.bytes, self.highlighted_offset).map(|val| val as u64),
    ...
    pub fn copy_absolute_address_at_highlighted(&mut self) {
        ...
        if let Some(clipboard) = &mut self.clipboard &&
            clipboard.set_text(format!("{:#X}", self.base_address + self.highlighted_offset)).is_ok()
        {
