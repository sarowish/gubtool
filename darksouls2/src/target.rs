...
impl Target {
    fn new() -> Self {
        let mut target = Self { chr_ctrl: None };
        target.update();
        target
    }
    ...
    pub fn update(&mut self) {
        match read_address(CaveAddress::SavedTargetPointer) {
            Ok(new_target) => {
                let same_target = self.chr_ctrl.as_ref()
    ...
    pub fn chr_ctrl(&mut self) -> ProcResult<&mut ChrCtrl> {
        self.chr_ctrl.as_mut().ok_or(ProcessError::null_pointer(PointerType::Target))
    }
...
static ACT_LOGGER: LazyLock<Mutex<ActLogger>> = LazyLock::new(|| {
    Mutex::new(ActLogger::default())
});
...
impl ToggleCommand for SaveTargetHook {
    ...
    fn set(&self, state: bool) -> anyhow::Result<()> {
        if state {
            ...
            write_addr_to_slice(&mut asm, fun.reloc("saved_ptr_loc"), CaveAddress::SavedTargetPointer)?;
            write_rel_i32(
                &mut asm,
            ...
            install_hook(&asm, CaveAddress::SaveTargetHook, Hook::LockedTargetPointer, orig_instr_len)?;
        } else {
...
impl ToggleCommand for ActHook {
    fn is(&self) -> ProcResult<bool> {
        if is_32() {
            read::<[u8; 7]>(Function::ChrSetAction)
                .map(|val| val != SET_ACT_ORIGINAL_VANILLA)
        } else {
            read::<[u8; 7]>(Function::ChrSetAction)
                .map(|val| val != SET_ACT_ORIGINAL_SCHOLAR)
        }
    }
    fn set(&self, state: bool) -> anyhow::Result<()> {
        if state {
            ...
            write_addr_to_slice(&mut asm, fun.reloc("repeating_chr_ai"), CaveAddress::ForceActChrAi)?;
