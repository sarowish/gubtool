...
use crate::{
    common::event_log::{EventLog, EventLogger, EventRecord},
...
impl Boss {
    ...
    pub fn revive_status(&self) -> &str {
        if !get_event_flag(self.death_flag).unwrap_or_default() { "Alive" } else { "Dead" }
    }
}
...
impl EventLogger for Ds2EventLogger {
    ...
    fn clear(&mut self) -> Result<()> {
        ...
        write_bytes(code_cave::base() + code_cave::EVENT_LOG_BUFFER, &[0x0; 0x1000])
    }
...
pub fn set_event_log_hook(state: bool) -> Result<()> {
    match state {
        ...
        false => {
            write_bytes(hooks::event_log(), &EVENT_LOG_HOOK_ORIGINAL)
        }
    }
}
...
fn install_event_log_hook_scholar() -> Result<()> {
    ...
    write_bytes(location, &asm)?;
    install_hook(location, hooks::event_log(), 5)
}
...
fn install_ivory_gauntlet_skip_scholar() -> Result<()> {
    ...
    write_to_slice::<u64>(&mut asm, fun.reloc("fn_get_map_object"), functions::get_map_obj_state_act_component())?;
    ...
    write_bytes(location, &asm)?;
    install_hook(location, functions::set_event(), 5)
}

fn install_ivory_gauntlet_skip_vanilla() -> Result<()> {
    let location = code_cave::base() + code_cave::IVORY_SKIP_HOOK;

    let mut asm = vanilla::ASM.get_function("ivory_skip").bytes.clone();
    ...
    write_bytes(location, &asm)?;
    install_hook(location, functions::set_event(), 6)
}
...
fn install_ivory_no_knights_scholar() -> Result<()> {
    ...
    let mut asm = scholar::ASM.get_function("ivory_knights").bytes.clone();
    write_rel_i32(&mut asm, location, 32, hooks::set_shared_flag() + 8, 4)?;

    write_bytes(location, &asm)?;
    install_hook(location, hooks::set_shared_flag(), 8)
}

fn install_ivory_no_knights_vanilla() -> Result<()> {
    let location = code_cave::base() + code_cave::IVORY_KNIGHTS_HOOK;

    let mut asm = vanilla::ASM.get_function("ivory_knights").bytes.clone();
    write_rel_i32(&mut asm, location, 28, hooks::set_shared_flag() + 7, 5)?;

    write_bytes(location, &asm)?;
    install_hook(location, hooks::set_shared_flag(), 7)
}
