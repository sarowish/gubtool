...
use ratatui::{
    ...
    style::Stylize,
    text::{Line, Span},
    widgets::{Cell, List, ListItem, Row, Table, TableState},
};

use crate::{
    common::event_log::EventLogger, config::ui_state::UiState, ds2::{
        event::{self, Ds2EventLogger, is_event_log_hook, set_event_log_hook},
        resources::{areas::AreaId, event_flags::EventFlag},
    }, send_input_event, tui::{
        common::{StrExt, block, controls_line, stateful_list::StatefulList, tab_state::TabState, tabs_list},
...
impl EventTab {
    ...
    pub fn draw(&mut self, frame: &mut Frame, layout: Rect) {
        ...
        let [_, controls_area] = Layout::default()
            ...
            .areas(layout[LOG_IDX]);
        frame.render_widget(controls_line(CONTROLS), controls_area);
    }

    pub fn handle_keys(&mut self, key: KeyEvent) {
        if self.tab.current_list == LOG_IDX {
            self.handle_log_table_keys(key);
        }
    ...
    fn handle_log_table_keys(&mut self, key: KeyEvent) {
        match (key.code, key.modifiers) {
            (KeyCode::Char('u'), KeyModifiers::CONTROL) => {
                *self.table_state.offset_mut() = self.table_state.offset().saturating_sub(28);
            }
            (KeyCode::Char('d'), KeyModifiers::CONTROL) => {
                *self.table_state.offset_mut() = self.table_state.offset().saturating_add(28);
            }
            ...
            (KeyCode::Char('c'), _) => self.log.clear().send_error(),
            (KeyCode::Char('x'), _) => {
                self.log.export()
    ...
    fn logs_table(&self) -> Table<'static> {
        let rows = self.log.get_entries().iter().enumerate().rev()
            .map(|(idx, record)| {
        ...
        Table::new(rows, widths).header(header).block(
            block(Some("Event Logs"), Some(self.tab.block_style(LOG_IDX)))
                .title(Self::logging_enabled_line().right_aligned()),
        )
    }

    fn logging_enabled_line() -> Line<'static> {
        if is_event_log_hook().unwrap_or_default() {
            Line::from("Enabled").style(theme().success)
        } else {
            Line::from("Disabled").style(theme().error)
        }
    }
}
        ...
        match self {
            ...
            Self::VisibleAava => {
                ...
                EventFlag::VisibleAava
                    .set_area_conditional_event(new_state, AreaId::FrozenEleumLoyce)
                    .send_error();
            }
            ...
            Self::BlackGulchChasmCleared => {
                ...
                EventFlag::BlackGulchChasmCleared
                    .set_area_conditional_event(new_state, AreaId::DarkChasmOfOld)
                    .send_error();
            }
            ...
            Self::ActivateBrume => {
                ...
                EventFlag::ActivateBrume
                    .set_area_conditional_event(new_state, AreaId::BrumeTower)
                    .send_error();
            }
        }
    }
