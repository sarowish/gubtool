use crate::{player, target, utility};
use config::{
    attach::{AttachEntry, apply_attach_entries},
    impl_attach_field_bool, impl_attach_field_f32,
};
...
pub async fn attach() -> anyhow::Result<()> {
    ...
    if attached::game() != Some(Game::EldenRing) {
        return Ok(())
    }
...
impl_attach_field_bool!(InfiniteConsumables, elden_ring, infinite_consumables, player::InfiniteConsumables);
...
impl_attach_field_bool!(DisableAreaWelcomeMessage, elden_ring, disable_area_welcome_message, utility::DisableAreaWelcomeMessage);
