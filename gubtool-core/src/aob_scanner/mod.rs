...
impl AobScanner {
    pub fn memory(scan: &'static AobScan) -> Result<Self, ScanError> {
        ...
        Ok(Self { scan, location, constraints, pattern, exhaustive: false })
    }
    ...
    where T: Into<PathBuf>
    {
        ...
        let file_with_path = FileWithPath { file, path };
        let location = ScanLocation::Disk(file_with_path);
        let constraints = make_scan_constraints(&location, &scan)?;
        Ok(Self { scan, location, constraints, pattern, exhaustive: false })
    }

    pub fn from_strategy(scan: &'static AobScan, strategy: ScanStrategy) -> Result<Self, ScanError> {
        match strategy {
            ScanStrategy::Disk(path) => {
                Self::disk(scan, path)
            }
            ScanStrategy::DiskExhaustive(path) => {
                Ok(Self::disk(scan, path)?
                    .make_exhaustive())
            }
            ScanStrategy::Mem => {
                Self::memory(scan)
            }
            ScanStrategy::MemExhaustive => {
                Ok(Self::memory(scan)?
                    .make_exhaustive())
            }
        }
...
fn make_scan_constraints(location: &ScanLocation, scan: &AobScan) -> Result<ScanConstraints, ScanError> {
    ...
    Ok(ScanConstraints { start, end, origin })
}