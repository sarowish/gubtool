...
pub fn dbg_lines() -> Vec<String> {
    vec![
        ...
        format!("target chr id: {}", target::target().chr_ins().and_then(|chr| chr.chr_id()).unwrap_or_default()),
        format!("target entity id: {}", target::target().chr_ins().and_then(|chr| chr.entity_id()).unwrap_or_default()),
    ]
}