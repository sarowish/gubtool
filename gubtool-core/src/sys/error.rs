...
#[derive(Debug, Copy, Clone)]
pub enum PtraceAction { Attach, Detach, Wait, Cont, GetRegs, SetRegs }
...
impl Display for ProcessError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match &self {
            Self::PartialAccess { access_type, bytes_accessed, address, location } => {
                let s = match access_type {
                    AccessType::Read(type_name) => format!("tried to read {type_name} at {:#X}, read {bytes_accessed} bytes", address),
                    AccessType::Write(WriteType::Type(type_name)) => format!("tried to write {type_name} at {:#X}, wrote {bytes_accessed} bytes", address),
                    AccessType::Write(WriteType::Bytes(num_written)) => format!("tried to write {num_written} bytes at {:#X}, wrote {bytes_accessed} bytes", address),
                };
                write!(
                    f, "{}:{}: {s}",
                    location.file(),
            ...
            Self::Io { access_type, address, location, .. } => {
                let s = match access_type {
                    AccessType::Read(type_name) => format!("failed to read {type_name} at {:#X} ({})", address, self.format_os_error()),
                    AccessType::Write(WriteType::Type(type_name)) => format!("failed to write {type_name} at {:#X} ({})", address, self.format_os_error()),
                    AccessType::Write(WriteType::Bytes(num_written)) => format!("failed to write {num_written} bytes at {:#X} ({})", address, self.format_os_error()),
                };
                write!(
                    f, "{}:{}: {s}",
                    location.file(),
