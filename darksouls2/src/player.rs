...
impl Player {
    fn new() -> Self {
        let mut player = Self { chr_ctrl: None };
        player.update();
        player
    }
    pub fn update(&mut self) {
        let ptr = ResolvedPtr::GameManagerImp.get()
    ...
    pub fn chr_ctrl(&mut self) -> ProcResult<&mut ChrCtrl> {
        self.chr_ctrl.as_mut().ok_or(ProcessError::null_pointer(PointerType::Player))
    }
    pub fn pointers(&self) -> Vec<(String, u64)> {
        self.chr_ctrl.as_ref().map(|c| c.pointers()).unwrap_or(Vec::default())
    }
}
...
impl ValueCommand::<u32> for Souls {
    ...
    fn set(&self, val: u32) -> anyhow::Result<()> {
        let souls_loc = player().chr_ctrl()?.get_ptr(ResolvedChrPtr::Stats)
            .add_offset(stats_offsets::SOULS);
        let current = souls_loc.read::<i32>()?;
        let diff = val.min(999999999) as i32 - current;
        if diff < 0 {
            souls_loc.write::<i32>(current as i32 + diff)?;
        } else {
            Self::give(diff)?;
        }
        Ok(())
...
impl ToggleCommand for InfinitePoise {
    ...
    fn set(&self, state: bool) -> anyhow::Result<()> {
        if state {
            ...
            let mut asm = fun.take_bytes();
            write_addr_to_slice(&mut asm, fun.reloc("game_man_imp"), BasePointer::GameManagerImp)?;
            write_rel_i32(&mut asm, CaveAddress::InfinitePoiseHook, fun.reloc("hook_loc"), Hook::InfinitePoise.add_offset(orig_instr_len), 4)?;
            install_hook(&asm, CaveAddress::InfinitePoiseHook, Hook::InfinitePoise, orig_instr_len)?;
        } else {
            let bytes: &[u8] = match is_32() {
...
pub(crate) static STATS: LazyLock<RwLock<Stats>> = LazyLock::new(|| {
    RwLock::new(Stats::default())
});
...
impl StatCommand for Stat {
    ...
    fn set(&self, val: u32) -> anyhow::Result<()> {
        ...
        let location = CaveAddress::LevelUpAsm;
